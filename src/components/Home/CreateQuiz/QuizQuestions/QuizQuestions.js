...
const QuizQuestions = ({ quizName, quizType }) => {
  ...
  const handleAddQuestion = async () => {
    ...
    if (!question.questionText.trim()) {
      toast.error("Name of question cannot be empty.");
      return;
    }
    ...
    if (question.correctOption === null || question.correctOption === '') {
      toast.error("select one correct option");
      return;
    }
    ...
    if (activeQuestion === questions.length) {
      ...
      toast.error(`First Add Question No. -${questions.length+1}`)
    }
  ...
  const displayQuestion = (index) => {
    ...
    if (index < questions.length) {
      ...
      // Reset to an empty question form for new question (when index equals questions.length)
      setQuestion({
        questionText: '',
        options: ['', ''],
        correctOption: null,
  ...
  const handleDeleteOption = (index) => {
    question.correctOption = question.correctOption === question.options[index] ? '' : question.correctOption
    const newOptions = [...question.options];
  ...
  const createQuiz = async () => {
    if (questions.length === 0 || question.questionText.trim() != ""){
      handleAddQuestion();
    }
